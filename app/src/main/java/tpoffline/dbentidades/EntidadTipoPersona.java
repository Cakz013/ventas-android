...
public class EntidadTipoPersona implements EntidadSincronizable {

    static final String SELECT_SOURCE = "select  descripcion, codtipopersona   from tipopersona   where estado = true";

    private static final String TBL_SOURCE_NAME = "tipopersona";
    ...
    public void sincronizar(Context context, int globalPartNumber,
        ...
        TipoPersonaDao tipoPersDao = daoSession.getTipoPersonaDao();
        ...
        try {
            ...
            while (rsAlianzaMainDB.next()) {
                ...
                long keyCampoIdentificadorRegistro = rsAlianzaMainDB.getInt("codtipopersona");

                String descripcionResgistro = rsAlianzaMainDB.getString("descripcion");


                TipoPersona  r = new TipoPersona(keyCampoIdentificadorRegistro, descripcionResgistro);

                long idNuevoInsertado = tipoPersDao.insert(r);

                MLog.d("Nuevo  objeto" + idNuevoInsertado + " " + r.toString());
    ...
    private void dropAndDeleteTable(SQLiteDatabase db) {
        ...
        if (dropAndDeleteTable) {
            ...
            TipoPersonaDao.dropTable(db, true);

            TipoPersonaDao.createTable(db, true);
        }
