...
public class EntidadZona implements EntidadSincronizable {

    static final String SELECT_SOURCE = "select  idzona, descripcion   from zona where estado = true";

    private static final String TBL_SOURCE_NAME = "zona";
    ...
    public void sincronizar(Context context, int globalPartNumber,
        ...
        ZonaDao barrioDao = daoSession.getZonaDao();
        ...
        try {
            ...
            while (rsAlianzaMainDB.next()) {
                ...
                long keyCampoIdentificadorRegistro = rsAlianzaMainDB.getInt("idzona");

                String descripcionResgistro = rsAlianzaMainDB.getString("descripcion");

                Zona z = new Zona(keyCampoIdentificadorRegistro, descripcionResgistro);

                long idNuevoInsertado = barrioDao.insert(z);
    ...
    private void dropAndDeleteTable(SQLiteDatabase db) {
        ...
        if (dropAndDeleteTable) {
            ...
            ZonaDao.dropTable(db, true);

            ZonaDao.createTable(db, true);
        }
